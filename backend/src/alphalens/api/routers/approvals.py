...
from alphalens.api.deps import ApprovalsServiceDep
...
def list_approvals(
    ...
    return service.list_approvals(status=status_filter)


@router.get("/{approval_id}", response_model=ApprovalRecord)
def get_approval(approval_id: str, service: ApprovalsServiceDep) -> ApprovalRecord:
...
def decide_approval(
    ...
    approval = service.decide_approval(approval_id, payload)
    if approval is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Approval '{approval_id}' not found.",
